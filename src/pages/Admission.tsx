...
const steps = [
  { step: '01', icon: <FileText size={28} />, title: 'Fill Application Form', desc: 'Download or collect the application form from school office or apply online.', color: 'bg-school-green', border: 'border-school-green' },
  { step: '02', icon: <FileText size={28} />, title: 'Submit Documents', desc: 'Submit birth certificate, address proof, photographs and previous report card.', color: 'bg-school-blue', border: 'border-school-blue' },
  { step: '03', icon: <Calendar size={28} />, title: 'Interaction Session', desc: 'Attend a short interaction session for the child and parents with our educators.', color: 'bg-school-orange', border: 'border-school-orange' },
  { step: '04', icon: <CheckCircle size={28} />, title: 'Confirmation & Fee', desc: 'Receive confirmation letter and complete fee payment to secure the seat.', color: 'bg-school-purple', border: 'border-school-purple' },
]
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 relative">
          ...
          {steps.map((s, i) => (
            ...
            >
              <div className={`w-16 h-16 ${s.color} text-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg`}>
                {s.icon}
              </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6 }}>
            ...
            <h2 className="text-2xl lg:text-3xl font-heading font-black text-gray-900 mb-6">
              Age Eligibility<br/>Session 2026-27
            </h2>
            <p className="text-gray-600 mb-6 text-sm">Age as on 31st March 2026 | As per NEP 2020 & Punjab Govt. norms</p>
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-lg border border-gray-100">
              ...
              {[
                ...
                <div key={cls} className={`flex justify-between items-center px-6 py-4 ${bg} border-b border-gray-100 last:border-0`}>
                  <span className="font-heading font-semibold text-gray-800">{cls}</span>
                  <span className="font-bold text-school-green bg-white px-3 py-1 rounded-full text-sm shadow-sm">{age}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }}>
            ...
            <div className="space-y-3 mb-8">
              {[
                ...
                <div key={doc} className="flex items-start gap-3 bg-white border border-gray-100 rounded-xl px-4 py-3 shadow-sm">
            ...
            <div className="bg-school-dark rounded-2xl p-6 text-white">
              ...
              <a href="tel:+918196051999" className="flex items-center gap-3 bg-school-gold text-school-dark px-5 py-3 rounded-lg font-bold hover:bg-yellow-400 transition-colors">
                <Phone size={18} />
                Call 81960-51999
              </a>
            </div>
...
function WhySection() {
  ...
  const inView = useInView(ref, { once: true })
  const reasons = [
    { icon: <BookOpen size={24} />, title: 'CBSE Curriculum', desc: 'Nationally recognized curriculum ensuring quality education.', color: 'bg-school-green' },
    { icon: <Users size={24} />, title: 'Expert Faculty', desc: '30+ qualified teachers dedicated to your child\'s growth.', color: 'bg-school-blue' },
    { icon: <CheckCircle size={24} />, title: 'Holistic Development', desc: 'Sports, arts, music and academics in perfect balance.', color: 'bg-school-orange' },
    { icon: <Calendar size={24} />, title: 'Safe Environment', desc: 'Secure, inclusive campus with trained staff.', color: 'bg-school-purple' },
  ]
  return (
    <section className="py-20 bg-white" ref={ref}>
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {reasons.map((r, i) => (
            ...
            >
              <div className={`w-14 h-14 ${r.color} text-white rounded-full flex items-center justify-center mx-auto mb-4`}>
                {r.icon}
              </div>
